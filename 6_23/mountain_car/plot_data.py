...
def plot_algo(data, name):
	mag_adv = [0.01, 0.02, 0.03, 0.05, 0.08, 0.1, 0.15, 0.2, 0.3]
	...
	for mean, std, algo in data:
		...
		# print 'std = ', std
		# print 'mean = ', mean
		plt.fill_between(mag_adv, mean - std, mean + std, alpha = 0.7)
	plt.xlabel('adv_mag')
	plt.ylabel('reward')
	plt.title(name)
	plt.legend()
...
# append the former 3 runs in folder 'result_data_0'
for j in range(3):
	
	for i in range(len(mag_adv)):
		path = 'result_data_0/adv_mag=' + str(mag_adv[i]) + '/adv_test_mag_{}_iteration_{}.pickle'.format(mag_adv[i], j)
		# print(path)
		with open(path, 'rb') as f: data = pickle.load(f)
